...
function TipsCard({ t, filteredCourses }) {
  return (
    <aside className="rounded-3xl border border-border/60 bg-surface2/90 p-6 shadow-e1">
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          ...
          {hasActiveFilters ? (
            <Button
              type="button"
              variant="ghost"
              className={CLEAR_FILTERS_CLASSES}
              onClick={() => {
              ...
              {trainingOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={
                      isActive ? ACTIVE_FILTER_CLASSES : INACTIVE_FILTER_CLASSES
                    }
...
function CourseList({ courses, onEdit, onPreview, onAssign, emptyMessage }) {
  if (courses.length === 0) {
    return (
      <section className="rounded-3xl border border-dashed border-border/60 bg-surface/60 p-12 text-center">
